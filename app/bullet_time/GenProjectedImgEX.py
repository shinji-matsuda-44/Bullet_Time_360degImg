...
class GenProjectedImgEX:
    ...
    def generateImg(self):
        ...
        for up in range(Wp):
            for vp in range(Hp):
                #p0の計算
                sight_vector = convTool.coordinate2vector(up, vp, Wp, Hp, dx, dy) #視線ベクトルp0
                rotated_sight_vector = np.dot(sight_rotation_matrix, sight_vector) #p0を回転したベクトル
                P = rotated_sight_vector
                #Qの計算
                unit_vector = rotated_view_axis/np.linalg.norm(rotated_view_axis) #単位ベクトル
                Q = unit_vector*(1-1/self.zoom_level)
                #QPの方向ベクトル(P-Q)
                m = (P-Q)/np.linalg.norm(P-Q) #単位ベクトル
                x0 = Q
                #パラメータtの計算
                R = 1 #半径は1の円
                D = np.square(np.dot(m,x0)) - (np.linalg.norm(x0)**2 - R**2)
                t = -np.dot(m,x0) + np.sqrt(D)
                #交点の位置(ベクトル)
                omae = x0 + np.dot(t,m)
                theta, phi = convTool.vector2angle(omae) #角度の計算
